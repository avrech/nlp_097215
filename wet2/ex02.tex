...
\end{itemize}
The basic feature set of model 1 achieved poor performance. The test accuracy upper bound was around 0.6. We analyzed the confusion matrix of model 1, and noticed that there are common mistakes which the parent POS was predicted correctly, but the distance between the child and parent was wrong. This was the motivation to introduce distance based features 14-17. The addition of features 14-17 achieved test accuracy of 0.78. We further analyzed the confusion matrix, and revealed that a considerable mass of the errors occur when the model predict a parent which is far from the child, while the true parent actually followed the child. We introduce the "punishment" feature (18), that punish false predictions such that. This last feature improved the test accuracy to stand on 0.8 after a small number of epochs.

We examined thresholding the sparsed features (4,8,10) in order to accelerate training. In fact, thresholding did not improved the training time significantly, and the performance seemed to be slightly worse, so we discarded this option. 
...
Table 1 shows the number of features of each type in each of the models.
